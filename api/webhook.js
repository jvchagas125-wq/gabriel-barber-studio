...
<head>
  ...
  <link rel="apple-touch-icon" href="https://i.postimg.cc/FFpNdnLT/Design-sem-nome.png">
  <meta name="application-name" content="Gabriel Barber Studio">
  <link rel="icon" type="image/png" href="https://i.postimg.cc/FFpNdnLT/Design-sem-nome.png">
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    html, body { min-height: 100%; color: var(--text); font-family: "DM Sans", sans-serif; background-color: var(--bg); background-image: url("https://i.imgur.com/rIkUnrQ.png"); background-size: cover; background-position: center top; background-attachment: fixed; background-repeat: no-repeat; }

    body::before{content:'';position:fixed;inset:0;z-index:-1;background:rgba(0,0,0,0.72);pointer-events:none;}
    ...
    .nav-inner{max-width:960px;margin:0 auto;padding:12px 16px;display:flex;align-items:center;justify-content:space-between;flex-wrap:nowrap;}
    .nav-links{display:flex;gap:4px;align-items:center;overflow-x:auto;-webkit-overflow-scrolling:touch;flex-shrink:1;min-width:0;}
    ...
    .profile-dropdown{position:absolute;top:calc(100% + 8px);right:0;min-width:280px;background:#1a1a1a;border:1px solid rgba(255,255,255,0.1);border-radius:12px;box-shadow:0 12px 40px rgba(0,0,0,.6);padding:8px;display:none;z-index:100;animation:fadeIn .15s ease-out;}
    .profile-wrap.open .profile-dropdown{display:block;}
    .profile-dd-header{padding:12px 14px 10px;border-bottom:1px solid rgba(255,255,255,0.06);margin-bottom:6px;}
    ...
    .service-features li{font-size:12px;color:var(--text-muted);display:flex;align-items:flex-start;gap:6px;line-height:1.5;word-break:normal;overflow-wrap:break-word;hyphens:none;}
    ...
    @media(max-width:640px){
      .nav-inner{padding:8px 12px;gap:4px;flex-wrap:nowrap;align-items:center;}
      ...
      .service-features li{font-size:11px;gap:4px;line-height:1.5;text-align:left;word-break:normal;overflow-wrap:anywhere;}
    ...
    /* ===== BANNERS / AVISOS ===== */
    .site-banner{display:flex;align-items:center;gap:10px;padding:8px 14px;border-radius:8px;margin-bottom:8px;font-size:13px;font-weight:500;animation:slideInUp .4s ease-out backwards;}
    ...
    /* ===== CARDS DE SERVIÇO — DESKTOP ===== */
    .service-card-header{display:flex;flex-direction:column;align-items:center;gap:0;margin-bottom:0;}
    .service-card-header img{width:56px;height:56px;object-fit:contain;margin:0 auto 12px;display:block;}
    .service-card-header-info{display:flex;flex-direction:column;align-items:center;gap:0;text-align:center;width:100%;}
    .service-card-header-info h3{font-weight:600;font-size:15px;margin-bottom:6px;}
    .service-card-header-info .service-price{font-family:"Bebas Neue",sans-serif;font-size:24px;color:var(--gold);}
    /* ===== CARDS DE SERVIÇO — MOBILE LISTA HORIZONTAL ===== */
    @media(max-width:640px){
      .services-grid{
      ...
      /* Lado esquerdo: ícone + nome + preço */
      .service-card-header{
      ...
      .service-card-header-info .service-price{
    ...
    /* Feats desktop/mobile switch */
    .svc-feats-desktop{ display:block; }
    .svc-feats-mobile { display:none;  }
    @media(max-width:640px){
      .svc-feats-desktop{ display:none !important; }
      .svc-feats-mobile { display:flex; flex:1; flex-direction:column; justify-content:center;
        padding-left:14px !important; border-left:1px solid rgba(201,168,76,0.3) !important; }
    }

    /* ===== INBOX DE NOTIFICAÇÕES ===== */
    .notif-badge{position:absolute;top:-4px;right:-4px;background:#dc2626;color:#fff;font-size:9px;font-weight:700;min-width:16px;height:16px;border-radius:8px;display:flex;align-items:center;justify-content:center;padding:0 3px;border:2px solid #111;display:none;}
    .notif-inbox{max-height:320px;overflow-y:auto;border-top:1px solid rgba(255,255,255,0.06);margin-top:6px;padding-top:6px;}
    .notif-item{padding:10px 12px;border-radius:8px;margin-bottom:4px;background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.05);cursor:pointer;transition:background .15s;}
    .notif-item:hover{background:rgba(255,255,255,0.06);}
    .notif-item.unread{background:rgba(201,168,76,0.08);border-color:rgba(201,168,76,0.2);}
    .notif-item-title{font-size:12px;font-weight:700;color:var(--text);margin-bottom:2px;}
    .notif-item-body{font-size:11px;color:var(--text-muted);line-height:1.4;}
    .notif-item-time{font-size:10px;color:var(--text-dim);margin-top:3px;}
    .notif-empty{text-align:center;padding:20px 0;color:var(--text-dim);font-size:12px;}

    /* Modal de nova notificação */
    .notif-modal{position:fixed;top:70px;left:50%;transform:translateX(-50%);z-index:500;width:calc(100% - 32px);max-width:420px;background:#1a1a1a;border:1px solid var(--gold-border);border-radius:14px;padding:16px 18px;box-shadow:0 12px 40px rgba(0,0,0,.7);animation:slideInDown .4s ease-out;display:none;}
    ...
    .notif-modal-header{display:flex;align-items:center;gap:10px;margin-bottom:8px;}
    .notif-modal-icon{font-size:24px;flex-shrink:0;}
    .notif-modal-title{font-weight:700;font-size:14px;color:var(--gold);flex:1;}
    .notif-modal-close{background:none;border:none;color:var(--text-muted);cursor:pointer;font-size:16px;padding:2px;}
    .notif-modal-body{font-size:13px;color:var(--text-muted);line-height:1.5;}
    .notif-modal-time{font-size:11px;color:var(--text-dim);margin-top:6px;}
    ...
    .date-picker-btn {
      ...
      color:var(--text-muted); font-size:15px; font-family:"DM Sans",sans-serif;
      cursor:pointer; text-align:left; display:flex; align-items:center;
      justify-content:space-between; gap:8px; transition:border-color .2s, color .2s;
    }
    .date-picker-btn:hover { border-color:var(--gold); color:var(--text); }
    .date-picker-btn.has-value { color:var(--text); border-color:var(--gold); }
    .date-picker-btn span { flex:1; }
    .date-picker-btn svg { flex-shrink:0; opacity:.5; }
    ...
    @media(max-width:768px){
      ...
      .profile-btn{ padding:4px !important; border-radius:50% !important; gap:0 !important; }
      .profile-wrap{ position:relative; }
      .profile-dropdown{ position:absolute; right:0 !important; left:auto !important; min-width:220px; top:calc(100% + 8px); }
...
<nav>
  <div class="nav-inner">
    ...
    <div class="nav-links">
      ...
      <button class="nav-btn" data-sec="ranking" onclick="showSection('ranking')">🏆 Ranking</button>
    ...
    <div class="profile-wrap" id="profile-wrap">
      ...
      <div class="profile-dropdown" id="profile-dropdown">
        ...
        <button class="profile-dd-btn" onclick="toggleNotifInbox()" style="justify-content:space-between;">
          <span>🔔 Notificações</span>
          <span id="notif-count-label" style="background:var(--gold);color:#111;font-size:10px;font-weight:700;padding:2px 7px;border-radius:10px;display:none;">0</span>
        </button>
        <!-- Inbox de notificações -->
        <div id="notif-inbox-wrap" style="display:none;">
          <div class="notif-inbox" id="notif-inbox-list">
            <div class="notif-empty">Nenhuma notificação ainda.</div>
          </div>
          <div style="display:flex;gap:6px;margin-top:6px;">
            <button onclick="markAllNotifsRead()" style="flex:1;padding:6px;border-radius:7px;border:none;background:rgba(201,168,76,0.1);color:var(--gold);font-size:11px;font-weight:700;cursor:pointer;">
              ✓ Lidas
            </button>
            <button onclick="clearAllNotifs()" style="flex:1;padding:6px;border-radius:7px;border:1px solid rgba(220,38,38,0.3);background:none;color:#f87171;font-size:11px;font-weight:700;cursor:pointer;">
              🗑️ Excluir
            </button>
          </div>
        </div>
...
<div class="section active" id="sec-home">
  <div class="page">
    <div class="hero">
      <div class="hero-logo"><img id="site-hero-logo" src="https://i.imgur.com/vcPQoRh.png" alt="Logo" onerror="this.style.display='none'"></div>
    </div>
  ...
  <div class="page" style="max-width:640px;">
    ...
    <div class="booking-card">
      ...
      <div class="date-picker-wrap">
        <button type="button" class="date-picker-btn" id="f-date-btn" onclick="openDatePicker('f-date','f-date-btn')">
          <span id="f-date-label">📅 Clique aqui para selecionar a data</span>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19 4h-1V2h-2v2H8V2H6v2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11zM7 11h2v2H7zm4 0h2v2h-2zm4 0h2v2h-2z"/></svg>
        </button>
        <input class="date-hidden" type="date" id="f-date" onchange="onDateChange('f-date','f-date-btn','f-date-label');renderSlots();" onkeydown="return false">
      </div></div>
...
<div class="section" id="sec-contact">
  <div class="page">
    ...
    <div class="contact-grid mb-8">
      <a id="contact-whatsapp" href="https://wa.me/5522997035391" target="_blank" rel="noopener" class="contact-card"><img src="https://i.imgur.com/gRZS5sC.png" alt="WhatsApp"><h3>WhatsApp</h3><p id="contact-whatsapp-num">(22) 99703-5391</p><p style="font-size:12px;margin-top:4px;">Clique para enviar mensagem</p></a>
      <a id="contact-instagram" href="https://instagram.com/gabriel.bstudio" target="_blank" rel="noopener" class="contact-card"><img src="https://i.imgur.com/o7nqXUj.png" alt="Instagram"><h3>Instagram</h3><p id="contact-instagram-handle">@gabriel.bstudio</p><p style="font-size:12px;margin-top:4px;">Siga nosso perfil</p></a>
      <a id="contact-address" href="https://maps.app.goo.gl/HLubhsnHgJELtAyQ7" target="_blank" rel="noopener" class="contact-card"><img src="https://i.imgur.com/lgZ7wqK.png" alt="Endereço"><h3>Endereço</h3><p id="contact-address-text">R. Luis Isaltino de Oliveira, 65<br>Pq Flamboyant - Campos dos Goytacazes<br>28015-170 (Loja 2)</p><p style="font-size:12px;margin-top:4px;">Clique para abrir no Maps</p></a>
    </div>
...
<div class="section" id="sec-home-vip">
  <div class="page" style="max-width:640px;">
    ...
    <div class="booking-card">
      ...
      <div class="form-group"><label class="form-label">Data</label>
      <div class="date-picker-wrap">
        <button type="button" class="date-picker-btn" id="vip-f-date-btn" onclick="openDatePicker('vip-f-date','vip-f-date-btn')">
          <span id="vip-f-date-label">📅 Clique aqui para selecionar a data</span>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor"><path d="M19 4h-1V2h-2v2H8V2H6v2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 16H5V9h14v11zM7 11h2v2H7zm4 0h2v2h-2zm4 0h2v2h-2z"/></svg>
        </button>
        <input class="date-hidden" type="date" id="vip-f-date" onchange="onDateChange('vip-f-date','vip-f-date-btn','vip-f-date-label');renderSlotsVip();" onkeydown="return false">
      </div></div>
...
<script type="module">
  ...
  function renderServicesGrid(){
    ...
    if(!grid) return;
    const btnStyle = 'background:none;border:none;color:#c9a84c;font-size:10px;font-weight:700;cursor:pointer;padding:2px 0;letter-spacing:.3px;';
    grid.innerHTML = SERVICES_DATA.map((s, si)=>{
      const allFeats = s.features || [];
      const MAX = 3;

      const featsFull = allFeats.length > 0
        ? '<ul class="service-features">' + allFeats.map(f=>'<li>'+f+'</li>').join('') + '</ul>'
        : '';

      let featsMobile = '';
      if(allFeats.length > 0){
        const visible = allFeats.slice(0, MAX);
        const hidden  = allFeats.slice(MAX);
        const visHtml = visible.map(f=>'<li>'+f+'</li>').join('');
        let hidHtml = '';
        hidden.forEach(f=>{
          hidHtml += '<li class="feat-hidden-'+si+'" style="display:none;">'+f+'</li>';
        });
        const btnHtml = hidden.length > 0
          ? '<li style="list-style:none;padding-left:0;margin-top:2px;">'
            + '<button onclick="toggleFeats('+si+', this)" style="'+btnStyle+'">'
            + '+ ver mais ('+hidden.length+')</button></li>'
          : '';
        featsMobile = '<ul class="service-features svc-feats-mobile-'+si+'">'
          + visHtml + hidHtml + btnHtml + '</ul>';
      }

      const imgHtml = s.image
        ? '<img src="' + s.image + '" alt="' + s.name + '" style="width:40px;height:40px;object-fit:contain;flex-shrink:0;">'
        : '';
      ...
      return '<div class="service-card">'
        ...
        + '</div>'
        + '</div>'
        + '<div class="svc-feats-desktop">' + featsFull + '</div>'
        + '<div class="svc-feats-mobile">'  + featsMobile + '</div>'
        + '</div>';
    }).join('');
  }

  window.toggleFeats = function(si, btn){
    const hidden = document.querySelectorAll('.feat-hidden-'+si);
    const isOpen = btn.dataset.open === '1';
    hidden.forEach(el=>{ el.style.display = isOpen ? 'none' : ''; });
    btn.dataset.open = isOpen ? '0' : '1';
    btn.textContent  = isOpen ? '+ ver mais ('+hidden.length+')' : '− ver menos';
  };
  ...
  window.cancelApt = async function(id){
    ...
    try {
      ...
      // Salvar no histórico de cancelados antes de deletar
      if(mainApt){
        const cancelId = 'cancel_' + id + '_' + Date.now();
        await setDoc(doc(db,'cancelled_appointments', cancelId),{
          ...mainApt,
  ...
  async function registerFCM(){
    try {
      // Registrar o SW do Firebase Messaging
      const swReg = await navigator.serviceWorker.register('/firebase-messaging-sw.js');
      await navigator.serviceWorker.ready;

      // Obter token FCM
      const token = await getToken(messaging, {
        vapidKey: 'BLwGsQRzcFrLs6XsSmgN3i2NP44Of28D9BrAjZtd_6XExonjtFCV_4KyY90Ee8cdltlz96TUflTxsmyHTDCgAEs',
        serviceWorkerRegistration: swReg,
      });

      if(token && loggedPhone){
        const digits = loggedPhone.replace(/\D/g,'');
        // Salvar token no Firestore vinculado ao telefone
      ...
      // Receber notificações com app em foreground
      onMessage(messaging, payload=>{
        const n = payload.notification || {};
        showPushNotification(
          n.title || 'Gabriel Barber Studio',
          n.body  || '',
          n.icon  || 'https://i.postimg.cc/FFpNdnLT/Design-sem-nome.png'
        );
      });

    } catch(e){ console.warn('FCM registerFCM error:', e); }
  }
    ...
    try {
      ...
      if(permission === 'granted'){
        ...
        setTimeout(()=>showPushNotification(
          ...
          'Notificações ativadas! Você receberá avisos mesmo com o app fechado.',
          'https://i.postimg.cc/FFpNdnLT/Design-sem-nome.png'
        ), 1000);
      } else {
  ...
  async function loadSiteBackground(){
    try {
      ...
      if(snap.exists() && snap.data().bgImage){
        document.body.style.backgroundImage = 'url("'+snap.data().bgImage+'")';
      }
    } catch(e){ console.warn('loadSiteBackground:',e); }
  }


  // ═══════════════════════════════════════
  // CONFIGURAÇÕES DO SITE (logo, contatos)
  // ═══════════════════════════════════════
  async function loadSiteConfig(){
    try {
      ...
      // Logo hero
      if(d.heroLogo){
        const img = document.getElementById('site-hero-logo');
        if(img) img.src = d.heroLogo;
      }
      // WhatsApp
      if(d.whatsappNum){
        const el = document.getElementById('contact-whatsapp');
        const num = document.getElementById('contact-whatsapp-num');
        if(el) el.href = 'https://wa.me/55'+d.whatsappNum.replace(/\D/g,'');
        if(num) num.textContent = d.whatsappNum;
      }
      // Instagram
      if(d.instagramHandle){
        const el = document.getElementById('contact-instagram');
        const handle = document.getElementById('contact-instagram-handle');
        const slug = d.instagramHandle.replace('@','');
        if(el) el.href = 'https://instagram.com/'+slug;
        if(handle) handle.textContent = d.instagramHandle.startsWith('@') ? d.instagramHandle : '@'+slug;
      }
      // Endereço
      if(d.addressText){
        const txt = document.getElementById('contact-address-text');
        if(txt) txt.innerHTML = d.addressText.replace(/\n/g,'<br>');
      }
      if(d.mapsLink){
        const el = document.getElementById('contact-address');
        if(el) el.href = d.mapsLink;
      }
  ...
  // ═══════════════════════════════════════
  window.openDatePicker = function(inputId, btnId){
    const input = document.getElementById(inputId);
    if(!input) return;
    // Tornar visível temporariamente para o browser abrir o picker
  ...
  window.onDateChange = function(inputId, btnId, labelId){
    const input = document.getElementById(inputId);
    ...
    if(input.value){
      ...
      const fmt = d.toLocaleDateString('pt-BR',{weekday:'long', day:'numeric', month:'long', year:'numeric'});
      label.textContent = fmt.charAt(0).toUpperCase() + fmt.slice(1);
      if(btn) btn.classList.add('has-value');
    } else {
      label.textContent = '📅 Clique aqui para selecionar a data';
  ...
  function startNotifListener(){
    ...
    notifUnsubscribe = onSnapshot(q, snap=>{
      ...
      // Verificar notificações não vistas
      const clearedAt = localStorage.getItem('gbs_notifs_cleared') || '';
      // Filtrar notificações posteriores ao último clear
      const visible = all.filter(n=> !clearedAt || (n.createdAt && n.createdAt > clearedAt));
      const unseen = visible.filter(n=> !localStorage.getItem('gbs_notif_read_'+n.id));

      // Atualizar badge
      const countLabel = document.getElementById('notif-count-label');
      if(countLabel){
        if(unseen.length > 0){
          countLabel.textContent = unseen.length > 9 ? '9+' : unseen.length;
      ...
      // Renderizar inbox
      renderNotifInbox(visible);

      // Mostrar modal para notificações novas (não no primeiro load)
      if(!firstLoad){
        snap.docChanges().forEach(change=>{
          if(change.type === 'added'){
            const n = change.doc.data();
            const key = 'gbs_notif_read_'+change.doc.id;
            if(!localStorage.getItem(key)){
              showNotifModal(n.title||'Gabriel Barber Studio', n.body||'', n.createdAt||'');
            }
          }
        });
      }

      // Ao entrar no site: mostrar modal da notificação mais recente não lida
      if(firstLoad && unseen.length > 0){
        const latest = unseen[0];
        // Só mostrar se não foi lida ainda
        if(!localStorage.getItem('gbs_notif_read_'+latest.id)){
          setTimeout(()=>{
            showNotifModal(latest.title||'Gabriel Barber Studio', latest.body||'', latest.createdAt||'', latest.id);
          }, 1500);
        }
      }
  ...
  function renderNotifInbox(notifications){
    ...
    const iconMap = {'servi':'✂️','plano':'💎','horário':'📅','aviso':'⚠️','promoção':'🏷️'};
    list.innerHTML = '';
    notifications.slice(0,20).forEach(n=>{
      const key = 'gbs_notif_read_'+n.id;
      const isRead = !!localStorage.getItem(key);
      const time = n.createdAt ? new Date(n.createdAt).toLocaleString('pt-BR',{day:'2-digit',month:'short',hour:'2-digit',minute:'2-digit'}) : '';
      const t = (n.title||'').toLowerCase();
      let icon = '🔔';
      for(const [k,v] of Object.entries(iconMap)){ if(t.includes(k)){icon=v;break;} }

      const div = document.createElement('div');
      div.className = 'notif-item' + (isRead ? '' : ' unread');
      div.innerHTML = '<div class="notif-item-title">'+icon+' '+(n.title||'Gabriel Barber Studio')+'</div>'
        + '<div class="notif-item-body">'+(n.body||'')+'</div>'
        + '<div class="notif-item-time">'+time+'</div>';
      div.onclick = function(){ readNotif(n.id, div); };
      list.appendChild(div);
    });
  }

  window.readNotif = function(id, el){
    localStorage.setItem('gbs_notif_read_'+id, '1');
    if(el) el.classList.remove('unread');
    // Atualizar badge
    const countLabel = document.getElementById('notif-count-label');
    if(countLabel){
      const unread = document.querySelectorAll('.notif-item.unread').length;
      countLabel.textContent = unread > 9 ? '9+' : unread;
      countLabel.style.display = unread > 0 ? 'inline-block' : 'none';
    }
  };

  window.clearAllNotifs = function(){
    if(!confirm('Excluir todas as notificações?')) return;
    // Salvar timestamp — só mostrar notificações APÓS esse momento
    const now = new Date().toISOString();
    localStorage.setItem('gbs_notifs_cleared', now);
    // Limpar inbox visual
    const list = document.getElementById('notif-inbox-list');
    if(list) list.innerHTML='<div class="notif-empty">Nenhuma notificação ainda.</div>';
    // Esconder inbox
    const wrap = document.getElementById('notif-inbox-wrap');
    if(wrap) wrap.style.display='none';
    // Zerar badge
    const badge = document.getElementById('notif-count-label');
    if(badge) badge.style.display='none';
    showToast('Notificações excluídas','success');
  };;
  ...
  window.markAllNotifsRead = function(){
    ...
    // Marcar todas no localStorage
    const list = document.getElementById('notif-inbox-list');
    if(list){
      list.querySelectorAll('[onclick^="readNotif"]').forEach(el=>{
        const match = el.getAttribute('onclick').match(/'([^']+)'/);
        if(match) localStorage.setItem('gbs_notif_read_'+match[1],'1');
      });
    }
    const countLabel = document.getElementById('notif-count-label');
  ...
  window.toggleNotifInbox = function(){
    const wrap = document.getElementById('notif-inbox-wrap');
    if(wrap) wrap.style.display = wrap.style.display==='none' ? 'block' : 'none';
  };

  function showNotifModal(title, body, createdAt, id){
    ...
    if(timeEl && createdAt){
      timeEl.textContent = new Date(createdAt).toLocaleString('pt-BR',{weekday:'long',day:'numeric',month:'long',hour:'2-digit',minute:'2-digit'});
    }
    modal.style.display='block';
    // Fechar automaticamente após 8 segundos
    clearTimeout(notifModalTimer);
    notifModalTimer = setTimeout(()=>closeNotifModal(), 8000);
    // Marcar como lida ao mostrar
    if(id) localStorage.setItem('gbs_notif_read_'+id,'1');
  }
  ...
  async function loadRanking(){
    try {
      const snap = await getDoc(doc(db,'config','ranking'));
      if(!snap.exists()) return;
      const { top30 } = snap.data();
      if(!top30 || !top30.length) return;
      renderRankingTop3(top30.slice(0,3));
      renderRankingList(top30);
    } catch(e){ console.warn('loadRanking:',e); }
  }

  function renderRankingTop3(top3){
    const el = document.getElementById('rank-top3');
    if(!el) return;
    const medals = ['🥇','🥈','🥉'];
    const colors = ['#c9a84c','#aaa','#cd7f32'];
    const order  = [1,0,2]; // 2º, 1º, 3º (pódio)
    el.innerHTML = order.map(i=>{
      const p = top3[i];
      if(!p) return '<div></div>';
      const isFirst = i===0;
      return '<div style="background:var(--bg2);border:1px solid rgba('+
        (isFirst?'201,168,76':'255,255,255')+',0.'+(isFirst?'3':'08')+');border-radius:14px;padding:18px 12px;text-align:center;'+(isFirst?'transform:translateY(-8px);':'')+'">'
        +'<div style="font-size:'+(isFirst?'40':'32')+'px;margin-bottom:8px;">'+medals[i]+'</div>'
        +'<div style="font-weight:700;font-size:13px;margin-bottom:4px;color:var(--text);">'+maskName(p.name)+'</div>'
        +'<div style="font-family:'Bebas Neue',sans-serif;font-size:22px;color:'+colors[i]+';">'+p.count+'</div>'
        +'<div style="font-size:10px;color:var(--text-dim);">'+(p.count===1?'visita':'visitas')+'</div>'
        +'</div>';
    }).join('');
  }
  ...
  function renderRankingList(top30){
    const el = document.getElementById('rank-list');
    if(!el) return;
    el.innerHTML = '<table style="width:100%;border-collapse:collapse;">'
      +'<thead><tr style="background:rgba(0,0,0,0.2);">'
      +'<th style="padding:10px 14px;text-align:left;font-size:11px;color:var(--text-dim);font-weight:700;">#</th>'
      +'<th style="padding:10px 14px;text-align:left;font-size:11px;color:var(--text-dim);font-weight:700;">CLIENTE</th>'
      +'<th style="padding:10px 14px;text-align:center;font-size:11px;color:var(--text-dim);font-weight:700;">VISITAS</th>'
      +'</tr></thead><tbody>'
      + top30.map((p,i)=>{
          const pos = i+1;
          const medal = pos===1?'🥇':pos===2?'🥈':pos===3?'🥉':'';
          const hl = pos<=3 ? 'background:rgba(201,168,76,0.05);' : '';
          return '<tr style="border-bottom:1px solid rgba(255,255,255,0.04);'+hl+'">'
            +'<td style="padding:11px 14px;font-size:13px;color:var(--text-muted);">'+( medal||pos+'º')+'</td>'
            +'<td style="padding:11px 14px;font-weight:600;font-size:13px;">'+maskName(p.name)+'</td>'
            +'<td style="padding:11px 14px;text-align:center;font-family:'Bebas Neue',sans-serif;font-size:20px;color:var(--gold);">'+p.count+'</td>'
            +'</tr>';
        }).join('')
      +'</tbody></table>';
  }
...
<div id="cancelVipModal" style="display:none;position:fixed;inset:0;z-index:400;background:rgba(0,0,0,0.85);align-items:center;justify-content:center;">
  <div style="width:100%;max-width:440px;margin:16px;background:#1a1a1a;border:1px solid rgba(220,38,38,0.35);border-radius:16px;padding:32px 28px;box-shadow:0 24px 80px rgba(0,0,0,.8);animation:slideUp .4s ease-out;">
    ...
    <p style="text-align:center;color:#888;font-size:13px;margin-bottom:12px;line-height:1.6;">Sentimos muito em ver você partir. Por favor, nos diga o motivo do cancelamento para que possamos melhorar.</p>
    <div style="background:rgba(234,179,8,0.1);border:1px solid rgba(234,179,8,0.4);border-radius:10px;padding:12px 16px;margin-bottom:20px;display:flex;align-items:flex-start;gap:10px;">
      <span style="font-size:18px;flex-shrink:0;">⚠️</span>
      <p style="font-size:12px;color:#fde047;line-height:1.6;margin:0;">Após a solicitação de cancelamento do seu plano atual, seu pedido será enviado à nossa central e, após confirmação, será oficialmente revogado.</p>
    </div>
...
<div id="notif-modal" class="notif-modal">
  <div class="notif-modal-header">
    ...
    <span class="notif-modal-title" id="notif-modal-title">Gabriel Barber Studio</span>
...
<div id="sec-ranking" class="section">
  <div class="page" style="max-width:640px;">
    <div style="text-align:center;padding:32px 0 24px;">
      <div style="font-family:'Bebas Neue',sans-serif;font-size:40px;background:linear-gradient(135deg,#c9a84c,#f0d78c);-webkit-background-clip:text;background-clip:text;-webkit-text-fill-color:transparent;">🏆 Ranking de Clientes</div>
      <div style="color:var(--text-muted);font-size:14px;margin-top:6px;">Os clientes mais fiéis da Gabriel Barber Studio</div>
    </div>

    <!-- TOP 3 destaque -->
    <div id="rank-top3" style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:24px;"></div>

    <!-- Lista TOP 30 -->
    <div style="background:var(--bg2);border-radius:14px;overflow:hidden;border:1px solid rgba(255,255,255,0.06);">
      <div id="rank-list" style="padding:0;">
        <div style="text-align:center;padding:32px;color:var(--text-dim);font-size:13px;">Carregando ranking...</div>
      </div>
    </div>
  </div>
</div>
